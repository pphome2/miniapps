...
function d_newdoc(){
  global $MA_SQL_RESULT,$D_NEW,$D_DOC_TABLE,$D_GO,$D_DB,$MA_ADMINFILE,
      $D_NEXT,$D_SEARCH_TEXT,$D_PAGEROW,$D_DOC_NEWFILE,$D_FILE_SEARCH,
      $D_PAGE_LEFT,$D_PAGE_RIGHT,$D_FILESTORE,$D_DOC_FILETABLE;

  if(isset($_POST['sfname'])){
    $sfn=$_POST['sfname'];
    $fl=glob("$D_FILESTORE/*$sfn*.pdf");
  }else{
    $sfn="";
  ...
  echo("<input type=text id=sfname name=sfname placeholder=\"$D_FILE_SEARCH\" value=\"$sfn\">");
  echo("</div>");
