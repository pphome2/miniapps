...
function r_outst1($rid){
  global $MA_SQL_RESULT,$R_INAME_TABLE_TITLE,$R_INAME_SEARCH,$R_TOOMANYROW,
...
function r_outst0(){
  global $MA_SQL_RESULT,$R_STR_TABLE_TITLE,$R_TOOMANYROW,$R_RAK_SEARCH,
  ...
  echo("<div class=frow>");
  echo("<div class=pcol1>");
  echo("<input type=text id=sname name=sname placeholder=\"$R_RAK_SEARCH\" value=\"$sn\">");
  echo("</div>");
  echo("<div class=pcol2>");
  echo("<div style=\"width:90%;float:middle;\">");
  echo("<span style=\"color:transparent;\">?</span>");
  ...
  echo("</div>");
  echo("<div class=pcol2>");
  echo("<input type=submit id=go name=go value=\"$R_WORK_RAK\">");
  echo("</div>");
  echo("</div>");
  echo("</form>");
  $page=0;
  ...
  $last=false;
  $odb=0;
  if (sql_run("select count(*) from r_raktar $sqlc;")){
    $r=$MA_SQL_RESULT[0];
    $odb=$r[0];
    $adb=$first+$R_PAGEROW;
    if ($adb>=$odb){
  ...
  echo("<input type=text id=search onkeyup='searchtable()' placeholder=\"$R_SEARCH\">");
  sql_run("select * from r_raktar $sqlc order by nev desc limit $first,$R_PAGEROW;");
  echo("<center>");
  echo("<table class='df_table_full' id=ptable>");
  echo("<tr class='df_trh'>");
